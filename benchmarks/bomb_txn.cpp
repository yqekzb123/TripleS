...
RC BombTxnManager::execute_phase(bool writes) {
  // Caracal can legitimately return WAIT while an earlier deterministic
  // version owns this row.  Keep the request cursor on that row so the
  // notification path resumes exactly where execution stopped; restarting
  // from zero would duplicate already-installed accesses and writes.
  BombQuery *bomb_query = static_cast<BombQuery *>(query);
  while (next_record_id < bomb_query->requests.size()) {
    BombRequest *request = bomb_query->requests[next_record_id];
    const uint64_t part = _bomb_wl->request_to_part(*request);
    if ((request->acctype == WR) != writes || GET_NODE_ID(part) != g_node_id) {
      ++next_record_id;
      continue;
    }
    RC rc = access_request(request, writes);
    if (rc != RCOK) return rc;
    ++next_record_id;
  }
  return RCOK;
...
#if CC_ALG == CARACAL
RC BombTxnManager::run_sub_caracal_txn() {
  // BoMB intentionally does not use Caracal's YCSB-only hot-key splitter.
  assert(false);
  return Abort;
}

RC BombTxnManager::run_caracal_txn() {
  BombQuery *bomb_query = static_cast<BombQuery *>(query);
  RC rc = RCOK;

  switch (caracal_phase) {
    case CARACAL_INIT:
      assert(simulation->caracal_phase.load() <= CARACAL_INIT_SYNC);
      // Caracal's INIT phase appends the deterministic writer version for
      // every local write. Reads do not need a version reservation.
      for (uint64_t i = 0; i < bomb_query->requests.size(); ++i) {
        BombRequest *request = bomb_query->requests[i];
        const uint64_t part = _bomb_wl->request_to_part(*request);
        bomb_query->partitions_touched.add_unique(part);
        if (request->acctype != WR || GET_NODE_ID(part) != g_node_id) continue;
        itemid_t *item = index_read(_bomb_wl->request_index(*request),
                                    request->key, part);
        assert(item != NULL && item->location != NULL);
        RC lock_rc = get_lock(static_cast<row_t *>(item->location), WR);
        if (lock_rc != RCOK) rc = lock_rc;
      }
      caracal_phase = CARACAL_EXECUTION;
      caracal_txn_phase = CARACAL_TXN_ANALYSIS;
      return rc;

    case CARACAL_EXECUTION:
      assert(simulation->caracal_phase.load() >= CARACAL_EXECUTION &&
             simulation->caracal_phase.load() <= CARACAL_EXECUTION_SYNC);
      while (rc == RCOK && caracal_txn_phase != CARACAL_TXN_DONE) {
        switch (caracal_txn_phase) {
          case CARACAL_TXN_ANALYSIS:
            bomb_query->get_participants(_bomb_wl);
            next_record_id = 0;
            caracal_txn_phase = CARACAL_TXN_RD;
            break;
          case CARACAL_TXN_RD:
            rc = execute_phase(false);
            if (rc == RCOK) {
              next_record_id = 0;
              caracal_txn_phase = CARACAL_TXN_WR;
            }
            break;
          case CARACAL_TXN_WR:
            // The dynamic prototype's guard changes do not alter its stable
            // logical slots, so execute at this already ordered version.
            // A future planner that changes keys must retry before INIT.
            rc = execute_phase(true);
            if (rc == RCOK) caracal_txn_phase = CARACAL_TXN_DONE;
            break;
          default:
            assert(false);
        }
