...
bool BombQueryGenerator::is_enabled_client(uint64_t client_thread) {
  ...
  // short sources for every paired client/server so no sequencer starves. A
  // node that does not own the global long lane uses that lane for short work;
  // otherwise it can stop with a permanently partial final batch.
  ...
  const uint64_t lane_count = std::min<uint64_t>(
      g_client_thread_cnt, BOMB_SHORT_WORKERS + (long_count == 0 ? 0 : 1));
