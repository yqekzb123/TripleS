...
void SDPCCSequencer::init(Workload * wl) {
	next_txn_id = 0;
...
void SDPCCSequencer::process_ack(Message * msg, uint64_t thd_id) {
	qlite_ll * en = wl_head;
	...
	uint64_t id = (msg->get_txn_id() % ARIA_BATCH_SIZE);
	// uint64_t id = (msg->get_txn_id() % ARIA_BATCH_SIZE) / g_node_cnt;
	...
	if (query_acks_left == 0) {
		...
		for(uint64_t i = 0; i < cl_msg->requests.size(); i++) {
			DEBUG_M("SDPCCSequencer::process_ack() ycsb_request free\n");
...
void SDPCCSequencer::process_abort(Message *msg, uint64_t thd_id) {
	qlite_ll * en = wl_head;
	uint64_t batch_id = msg->get_batch_id();
	// uint64_t batch_id = msg->get_batch_id();
	while(en != NULL && en->epoch != batch_id) {
...
// Assumes 1 thread does sequencer work
void SDPCCSequencer::process_txn(Message *msg, uint64_t thd_id, uint64_t early_start,
	...
	uint64_t batch_id = en->epoch * g_node_cnt + g_node_id;
	txnid_t txn_id = batch_id * ARIA_BATCH_SIZE + next_txn_id;
	// g_node_id + g_node_cnt * next_txn_id;
	next_txn_id++;
	uint64_t id = (txn_id % ARIA_BATCH_SIZE);
	// uint64_t id = (txn_id % ARIA_BATCH_SIZE) / g_node_cnt;
...
// Assumes 1 thread does sequencer work
void SDPCCSequencer::send_next_batch(uint64_t thd_id) {
	uint64_t prof_stat = get_sys_clock();
	qlite_ll * en = wl_tail;
#if LOGGING
