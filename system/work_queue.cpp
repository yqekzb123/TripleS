...
void QWorkQueue::init() {
	...
	sched_ready = true;
	calvin_scheduled_list_lockfree = new LockList<list_node_entry *>("CalvinScheduledList");
	#if CC_ALG == ARIA
		read_ready = true;
		aria_read_lockfree = new LockList<list_node_entry *>("AriaReadList");
		reserve_ready = true;
		aria_reserve_lockfree = new LockList<list_node_entry *>("AriaReserveList");
		check_ready = true;
		aria_check_lockfree = new LockList<list_node_entry *>("AriaCheckList");
		commit_ready = true;
		aria_commit_lockfree = new LockList<list_node_entry *>("AriaCommitList");
	#endif
#endif
#if CC_ALG == ARIA
...
#if LONG_TXN_SCHEDULE
// 需要写一个最基本的lock-free list插入函数，需要指定线程号，插入的链表，还有插入的消息
...
void QWorkQueue::insert_list_lockfree(uint64_t thd_id, TxnManager * txn) {
...
TxnManager * QWorkQueue::get_from_list_lockfree(uint64_t thd_id, uint64_t &key) {
	// 第一个函数
	...
	if (succ) {
		// DEBUG("[LockFreeList] thd %ld get_from_list_lockfree key=%lu txn=%p\n", thd_id, entry->key, entry->txn);
	...
	if (!valid) {
		...
		case ARIA_READ:
		{
			std::function<bool(list_node_entry*)> func = [](list_node_entry * arg) -> bool {
				list_node_entry * entry = arg;
				if (!entry) return false;
				if (entry->key <= min_read_reservation_sid) {
					return true;
				}
				DEBUG_LOCKFREE("[LockFreeList] get_from_calvin_list_lockfree cond1 skip txn %p key=%lu minSid=%lu\n",entry->txn, entry->key, min_read_reservation_sid);
				return false;
			};
			
			valid = aria_read_lockfree->try_take(func, func2, entry2, thd_id);
			// valid = aria_read_queue->pop(entry);
			if (valid) {
				// printf("thd_id: %ld pop txn: %ld from read queue\n", thd_id, entry->msg->txn_id);
			}
		...
		case ARIA_RESERVATION:
		{
			std::function<bool(list_node_entry*)> func = [](list_node_entry * arg) -> bool {
				list_node_entry * entry = arg;
				if (!entry) return false;
				if (entry->key <= min_reservation_check_sid) {
					return true;
				}
				DEBUG_LOCKFREE("[LockFreeList] get_from_calvin_list_lockfree cond1 skip txn %p key=%lu minSid=%lu\n",entry->txn, entry->key, min_reservation_check_sid);
				return false;
			};
			valid = aria_reserve_lockfree->try_take(func, func2, entry2, thd_id);
			// valid = aria_reserve_queue->pop(entry);
			if (valid) {
				// printf("thd_id: %ld pop txn: %ld from reserve queue\n", thd_id, entry->msg->txn_id);
			}
		...
		case ARIA_CHECK:
		{
			std::function<bool(list_node_entry*)> func = [](list_node_entry * arg) -> bool {
				list_node_entry * entry = arg;
				if (!entry) return false;
				if (entry->key <= min_check_commit_sid) {
					return true;
				}
				DEBUG_LOCKFREE("[LockFreeList] get_from_calvin_list_lockfree cond1 skip txn %p key=%lu minSid=%lu\n",entry->txn, entry->key, min_check_commit_sid);
				return false;
			};
		...
		case ARIA_COMMIT:
		{
			std::function<bool(list_node_entry*)> func = [](list_node_entry * arg) -> bool {
				list_node_entry * entry = arg;
				if (!entry) return false;
				if (entry->key <= min_commit_read_sid) {
					return true;
				}
				DEBUG_LOCKFREE("[LockFreeList] get_from_calvin_list_lockfree cond1 skip txn %p key=%lu minSid=%lu\n",entry->txn, entry->key, min_commit_read_sid);
				return false;
			};
