...
std::vector<std::vector<ConflictScore>> Reorder::calc_conflict_matrix(const std::vector<Message*>& txn_list) {
	...
	// 分别提取每个事务的读集和写集
	uint64_t *pstats = cc_selector.pstats;
...
std::vector<Message*> Reorder::schedule_transactions_advanced(const std::vector<Message*>& txn_list, int delta, double lambda, int max_batch_size) {
	...
	std::vector<std::unordered_set<uint64_t>> readsets(N), writesets(N);
	// 说明：构建每个子事务的读集和写集（以 key 为单位）。后续的冲突计算
	// 将直接基于这些集合进行交集检查。对于 YCSB，每个 request 包含 key 与访问类型。
	for (size_t i = 0; i < N; ++i) {
#if WORKLOAD == YCSB
		YCSBClientQueryMessage* msg = (YCSBClientQueryMessage*)txn_list[i];
		for (size_t j = 0; j < msg->requests.size(); ++j) {
			uint64_t key = msg->requests[j]->key;
			if (msg->requests[j]->acctype == RD) readsets[i].insert(key);
			else if (msg->requests[j]->acctype == WR) writesets[i].insert(key);
		}
#endif
	}

	// 说明：这里预定义了不同冲突类型的权重，后面的冲突计算会基于这些权重并乘以 shard 热度。
	// 选择的权重参考了你提供的模型（WW:2.0, WR:1.5, RW:1.0, RR:0.1），此处 rr 被设为 0.0（可调整）。
	const double w_ww = 2.0;
	const double w_wr = 1.5;
	const double w_rw = 1.0;
	const double w_rr = 0.0;
	...
	// 便于在计算冲突时快速定位原始读写集合。
	std::vector<size_t> output_indices; output_indices.reserve(N);

	// 构建 Message* -> 原始下标 映射，便于检查依赖（depends_on_messages）
	std::unordered_map<Message*, size_t> msg_to_idx;
	for (size_t i = 0; i < N; ++i) msg_to_idx[txn_list[i]] = i;
	for (size_t placed = 0; placed < N; ++placed) {
		...
		for (size_t i = 0; i < N; ++i) {
			...
			// 依赖检查：如果事务 i 在 message 层声明了依赖（depends_on_messages），
			// 则只有当其所有依赖都已被放入 output（chosen == true）时，才允许选择 i。
			// 这可以保证在重排序过程中，子事务的 step/依赖关系不会被破坏。
			#if WORKLOAD == YCSB
			YCSBClientQueryMessage * check_msg = (YCSBClientQueryMessage *) txn_list[i];
			if (!check_msg->deps_left > 0) continue; // 依赖未满足，跳过该候选
			#endif
			...
			for (int back = (int)output_indices.size() - 1; back >= 0 && k <= delta; --back, ++k) {
				...
				// 写-写冲突：i 写 && idx_j 写
				for (auto key : writesets[i]) {
					if (writesets[idx_j].count(key)) {
						uint64_t shard = key_to_shard(key);
						double hot = pstats ? (double)pstats[shard] : 1.0;
						w += w_ww * hot; // 按 WW 权重并乘以热度
					}
				}
				// 写-读冲突：i 写 && idx_j 读
				for (auto key : writesets[i]) {
					if (readsets[idx_j].count(key)) {
						uint64_t shard = key_to_shard(key);
						double hot = pstats ? (double)pstats[shard] : 1.0;
						w += w_wr * hot;
					}
		...
		if (best_idx == -1) break; // nothing left (shouldn't happen)
	...
	// Note: we don't return the new delta here; caller may query stats or we can
	// expose an API to retrieve tuned delta if desired. For now we just return
