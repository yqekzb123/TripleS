...
RC Row_sdocc::check(TxnManager * txn, access_t type, row_t * local_row, Access *a) {
    ...
    if (type == RD || type == SCAN) {
        ...
        if (txn->last_sdocc_write_reservation > a->sdocc_write_reservation) {
            ...
            DEBUG_WRK("[SDOCC] txn %ld,%ld read key %ld, write reservation from %ld to %ld, read reservation from %ld to %ld, return RCOK\n", txn->get_batch_id(), txn->get_txn_id(), key, a->sdocc_write_reservation, txn->last_sdocc_write_reservation, a->sdocc_read_reservation, txn->last_sdocc_read_reservation);
        }
    } else if (type == WR) {
        // 写操作，检查读操作和写操作，即处理写写冲突和写读冲突
        if ((txn->last_sdocc_write_reservation > a->sdocc_write_reservation && 
                txn->last_sdocc_write_reservation != key) || 
            txn->last_sdocc_read_reservation > a->sdocc_read_reservation) {
            ...
            DEBUG_WRK("[SDOCC] txn %ld,%ld write key %ld, write reservation from %ld to %ld, read reservation from %ld to %ld, return RCOK\n", txn->get_batch_id(), txn->get_txn_id(), key, a->sdocc_write_reservation, txn->last_sdocc_write_reservation, a->sdocc_read_reservation, txn->last_sdocc_read_reservation);
        }
    } else {
        // 其他操作
        assert(false);
