...
for HOSTNAME in ${HOSTS}; do
    ...
    if [ $count -ge $NODE_CNT ]; then
        SCRIPT="ulimit -c unlimited; export LD_LIBRARY_PATH=${LDLIB}:$LD_LIBRARY_PATH; env SCHEMA_PATH=\"$2\" timeout -k 9 10m ${PATHE}runcl -nid${count} > ${PATHE}clresults.out 2>&1"
        echo "${HOSTNAME}: runcl ${count}"
    else
        # SCRIPT="env SCHEMA_PATH=\"$2\" timeout -k 9 10m ${PATHE}rundb -nid${count} > ${PATHE}dbresults.out 2>&1"
        SCRIPT="ulimit -c unlimited; export LD_LIBRARY_PATH=${LDLIB}:$LD_LIBRARY_PATH; env SCHEMA_PATH=\"$2\" ${PATHE}rundb -nid${count} > ${PATHE}dbresults.out 2>&1"
